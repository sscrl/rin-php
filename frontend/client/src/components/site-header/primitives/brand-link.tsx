...
export function BrandLink({
  ...
  const brandImage = siteConfig.logo || siteConfig.avatar;
  const isLogo = Boolean(siteConfig.logo);

  return (
    <Link aria-label="home" href="/" className={className}>
      {showAvatar && brandImage ? (
        <img
          src={brandImage}
          alt={isLogo ? "Logo" : "Avatar"}
          className={
            isLogo
              ? compact
                ? "h-10 w-auto max-w-[9rem] object-contain"
                : "h-12 w-auto max-w-[10rem] object-contain"
