...
export function PreviewBrand({ data, compact = false }: { data: HeaderLayoutPreviewData; compact?: boolean }) {
  return (
    <div className="flex items-center gap-2 overflow-hidden">
      <PreviewAvatar avatar={data.logo || data.avatar} themeColor={data.themeColor} compact={compact} contain={Boolean(data.logo)} />
...
function PreviewAvatar({
  ...
  return avatar ? (
    <img
      src={avatar}
      alt=""
      className={`${contain ? (compact ? "h-7 w-auto max-w-[4.5rem] rounded-lg object-contain" : "h-8 w-auto max-w-[5rem] rounded-lg object-contain") : compact ? "h-7 w-7 rounded-xl object-cover" : "h-8 w-8 rounded-full object-cover"} shrink-0`}
    />
  ) : (
