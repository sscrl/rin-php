...
export function Settings() {
  ...
  const previewSiteLogo = String(clientConfig.get("site.logo") ?? clientConfig.default("site.logo") ?? "");
    ...
    <div className="flex w-full flex-col">
      ...
      <main className="w-full rounded-2xl bg-w" aria-label={t("main_content")}>
        <div className="flex flex-col items-start space-y-2">
          ...
          />
          <ItemImageInput
            title={t("settings.site.logo.title")}
            description={t("settings.site.logo.desc")}
            configKeyTitle={t("settings.site.logo.label")}
            value={String(clientConfig.get("site.logo") ?? "")}
            placeholder={String(clientConfig.default("site.logo") ?? t("settings.site.logo.label"))}
            onChange={(value) => {
              setConfigValue("client", "site.logo", value);
            }}
            onError={showAlert}
            fit="contain"
          />
